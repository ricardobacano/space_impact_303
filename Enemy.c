...
void check_collision_with_player(square *player, Enemy **enemies) {
    ...
    while (current != NULL) {
        // Pega os limites da hitbox do inimigo
        ...
        // Pega os limites da hitbox do player
        ...
        // Checa se as hitboxes se sobrepõem
        if (player_right >= enemy_left && player_left <= enemy_right &&
            player_bottom >= enemy_top && player_top <= enemy_bottom) {
            
            int damage = 10; // Dano causado pelo inimigo

            if (player->shield && player->shield->is_active) { // Verifica se o escudo existe e está ativo
